...
import {
  CalendarCheck, TrendingUp, Users, Sparkles, Star, CircleDot, Bell, Boxes, Gift,
} from "lucide-react"
...
export function DashboardView() {
  ...
  const kpis = [
    ...
    { label: "Hatırlatma bekliyor", value: stats?.v4 ? String(stats.v4.remindersDue) : "…", icon: Bell, tone: stats?.v4 && stats.v4.remindersDue > 0 ? "text-amber-400" : "text-muted-foreground" },
    { label: "Düşük stok", value: stats?.v4 ? String(stats.v4.lowStock) : "…", icon: Boxes, tone: stats?.v4 && stats.v4.lowStock > 0 ? "text-red-400" : "text-muted-foreground" },
    { label: "Haftalık doluluk", value: stats ? `%${stats.week.utilization}` : "…", icon: TrendingUp, tone: "text-brand-text" },
    { label: "Ödül hazır", value: stats?.v4 ? String(stats.v4.loyaltyReady) : "…", icon: Gift, tone: stats?.v4 && stats.v4.loyaltyReady > 0 ? "text-brand-text" : "text-muted-foreground" },
  ]
    ...
    <div className="mk-velvet">
      ...
      <section className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {kpis.map((kpi, i) => (
            <div key={kpi.label} className={cn("mk-card mk-kpi mk-anim-up relative overflow-hidden rounded-xl p-5", `mk-delay-${Math.min(6, i + 1)}`)}>
