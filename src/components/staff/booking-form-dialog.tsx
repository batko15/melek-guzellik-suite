...
import {
  CalendarPlus, Pencil, Trash2, Loader2, Clock, History, UserRound, Sparkles, Phone, Mail, AlertTriangle, Banknote, Users2,
} from "lucide-react"
...
import {
  BOOKING_STATUS, type SalonBooking, type SalonService, type StaffMemberRow, openSlots,
...
export function BookingFormDialog({
  ...
  // V4: Ekip üyeleri (atama için)
  const { data: staffData } = useQuery({
    queryKey: ["salon-staff-assign"],
    queryFn: async () => {
      const res = await fetch("/api/v1/salon/staff")
      if (!res.ok) return { staff: [] as StaffMemberRow[] }
      return (await res.json()) as { staff: StaffMemberRow[] }
    },
  })
  const staffMembers = useMemo(() => (staffData?.staff ?? []).filter((s) => s.active), [staffData])
  ...
  useEffect(() => {
    ...
    if (booking) {
      ...
      setDeposit(Math.round(booking.deposit ?? 0))
  ...
  const save = async () => {
    ...
    if (deposit < 0 || deposit > 100000) return setError("Depozito 0–100.000 arasında olmalı.")
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="mk-scroll max-h-[92vh] w-[calc(100vw-2rem)] max-w-2xl overflow-y-auto rounded-2xl border-border">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {/* V4: Depozito & ekip ataması */}
            <div className="space-y-1.5">
              <Label htmlFor="rf-deposit" className="flex items-center gap-1.5"><Banknote className="h-3.5 w-3.5 text-brand-text" /> Depozito (₺)</Label>
              <div className="flex items-center gap-2">
                <Input id="rf-deposit" type="number" min={0} max={100000} step={50} value={deposit}
                  onChange={(e) => setDeposit(Number(e.target.value))} className="mk-focus h-11 rounded-xl" />
                <button
                  type="button"
                  onClick={() => setDepositPaid(!depositPaid)}
                  className={cn(
                    "mk-focus flex h-11 shrink-0 items-center gap-1.5 rounded-xl border px-3 text-[11px] font-bold transition-colors",
                    depositPaid
                ...
                >
                  {depositPaid ? "✓ Alındı" : "Alınmadı"}
                </button>
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="flex items-center gap-1.5"><Users2 className="h-3.5 w-3.5 text-brand-text" /> Ekip ataması</Label>
              <Select value={staffId || "yok"} onValueChange={(v) => setStaffId(v === "yok" ? "" : v)}>
                <SelectTrigger className="mk-focus h-11 rounded-xl">
                  <SelectValue placeholder="Atanmadı" />
                </SelectTrigger>
                <SelectContent className="max-h-60 rounded-xl">
                  <SelectItem value="yok">— Atanmadı —</SelectItem>
                  {staffMembers.map((m) => (
                    <SelectItem key={m.id} value={m.id}>
                      {m.name} · {m.role} · %{m.commissionRate}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            {/* V4: Alerji uyarısı (düzenlemede — müşteri kartından) */}
            {isEdit && (booking!.customer.allergies || booking!.customer.sensitive) && (
              <div className="rounded-xl border border-red-800/60 bg-red-950/30 px-3.5 py-3 sm:col-span-2">
                <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-red-300">
                  <AlertTriangle className="h-3.5 w-3.5" /> Alerji & hassasiyet — dikkat!
                </div>
                {booking!.customer.allergies && (
                  <p className="mt-1 text-xs font-semibold leading-relaxed text-red-200">{booking!.customer.allergies}</p>
                )}
                {booking!.customer.sensitive && (
