// Ekip Portalı — Galeri Yönetimi: açılış sayfasında görünen görseller

"use client"

import { useQuery } from "@tanstack/react-query"
import { Image as ImageIcon, Instagram, Eye } from "lucide-react"
      ...
      <section className="border-b border-border/60">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-primary/40 bg-primary/10 px-3 py-1 text-xs font-semibold text-brand-text">
            <ImageIcon className="h-3.5 w-3.5" /> Çalışmalar
          </div>
          <h1 className="mk-display text-2xl font-bold tracking-tight sm:text-3xl">
            Galeri <span className="mk-gold-text">yönetimi</span>
          </h1>
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            {items.length} görsel herkese açık web sayfasında («Galeri» bölümü) bu sırayla görünüyor.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
        <div className="mk-card flex flex-col items-start gap-3 rounded-xl border-primary/25 p-4 sm:flex-row sm:items-center">
          ...
          <p className="flex-1 text-xs leading-relaxed text-muted-foreground">
            Bu görseller açılış sayfasında görünür. Kendi fotoğraflarınızı PNG/JPG olarak{" "}
            <code className="rounded bg-secondary px-1.5 py-0.5 font-mono text-[10px] text-foreground">public/gallery/</code>{" "}
            klasörüne koyup veritabanına ({" "}
            <code className="rounded bg-secondary px-1.5 py-0.5 font-mono text-[10px] text-foreground">GalleryItem</code>{" "}
            tablosu) kaydedin — ya da mevcut dosyaları değiştirin.
          </p>
          <a
            href={instagramUrl}
            target="_blank"
          ...
          >
            <Instagram className="h-3.5 w-3.5 text-brand-text" /> Instagram'ı aç
          </a>
        </div>
              ...
              <figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-background/90 to-transparent px-3 pb-2.5 pt-8">
                <div className="flex items-center justify-between gap-1">
                  ...
                  <Badge className="shrink-0 border-primary/30 bg-background/70 text-[9px] text-brand-text">
                    <Eye className="mr-0.5 h-2.5 w-2.5" /> Yayında
                  </Badge>
                </div>
              </figcaption>
